...
function! KnoteOpenSettings()
	silent execute 'tabe '.s:rootpath.'/settings.txt'
endfunction

function! UpdateList()
    let l:KnoteTree = {
                \ "General": {
                \   "notes": systemlist("ls -p " . s:rootpath . "|grep -v /$|grep -v settings.txt"),
...
if !isdirectory(s:rootpath)
	...
	silent call writefile(["# Initial settings auto-generated by knote"], s:rootpath."/settings.txt")
	silent call writefile(s:settings, s:rootpath."/settings.txt", "a")
endif
