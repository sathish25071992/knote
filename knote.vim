...
\	"    \\S - Open settings                      ",
\	"    \\y - Sync the notes with repository     " 
\]

let s:settings = ["giturl=", "sshkey="]
...
function! KnoteGitSync()
	if has_key(s:settings, "giturl") && executable("git") == 1
		echohl WarningMsg | echomsg "Syncing the repository... This might take some time..." | echohl None
		call system("git add .")
		call system("git commit -m 'syncing the changes'")
		call system("git pull")
		call system("git push")
		call UpdateList()
		call KnoteListRender()
		redraw
		echomsg "Syncing complete!"
	endif
endfunction
...
function! UpdateList()
    let l:KnoteTree = {
                \ "General": {
                \   "notes": systemlist("ls -p " . s:rootpath . "|grep -v /$|grep -v settings.json"),
...
let s:rootpath = fnamemodify(s:rootpath, ":p")
vertical new
vertical resize 30
...
nnoremap <silent> <Leader>S :call KnoteOpenSettings()<cr>
nnoremap <silent> <Leader>y :call KnoteGitSync()<cr>

let s:statuswin = win_getid()
let s:mainwin = win_getid(winnr('$'))
call KnotePopulateBanner(s:mainwin, s:banner)

if !isdirectory(s:rootpath)
	silent call mkdir(s:rootpath, 'p')
	silent call writefile(['{"version": "1.0"}'], s:rootpath."/settings.json")
endif
if filereadable(s:rootpath.'/settings.json')
	let rawsettings = readfile(s:rootpath.'/settings.json')
	try
		let s:settings = json_decode(join(rawsettings))
	catch
		echohl ErrorMsg | echo "Invalid setting detected. Please check the content is proper json format" | echohl None
	endtry
endif
...
if has_key(s:settings, "giturl") && executable("git") == 1
	" Git url is given. Try to get the repo
	if !isdirectory(s:rootpath.'.git')
		echo "Syncing the note repository..."
		silent execute "!git init"
		silent execute "!git remote add origin ".s:settings["giturl"]
		silent execute "!git pull origin master"
		silent execute "!git push --set-upstream origin master"
		silent call writefile(["settings.json"], s:rootpath.'.gitignore', "s")
		silent call writefile(["*.swp"], s:rootpath.'.gitignore', "as")
	else
		let remote = system('git remote get-url origin')
		if remote !~ s:settings["giturl"]
			silent execute "!git remote set-url origin ".s:settings["giturl"]
			echo "Syncing the note repository..."
			silent execute "!git pull origin master"
		endif
	endif
